...
public class AlarmReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (intent.getAction().equals(Const.INTENT_ACTION_CREATE_NEW_TASKS)) {
            // 월요일 00:00 보다 데드라인 날짜가 큰 경우만
            List<GoalEntity> goalEntities = mRequery.getData().select(GoalEntity.class).where(GoalEntity.DEAD_LINE_DATE.gt(new Date())).get().toList();
